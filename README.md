...
- **WebMCP** — the landing page registers `get_wrong_answer` as a client-side tool via `document.modelContext.registerTool()` (feature-detected; early preview, Chromium 146+ behind a flag), reusing the exact same `wrongify()` logic the chat widget already runs — for in-browser AI agents that can drive the page directly, no server round trip.
